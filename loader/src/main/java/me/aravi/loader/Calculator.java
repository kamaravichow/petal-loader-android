...
import me.aravi.loader.components.Coodinator;

public class Calculator {

    private final double[] mCosValues;
    private final double[] mSinValues;

    public Calculator(int segmentCount) {
    ...
    public List<Coodinator> getSegmentsCoordinates(int rectSize, int outPadding, int inPadding, int segmentCount, int finalWidth) {

        List<Coodinator> coordinates = new ArrayList<>(segmentCount);
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            Coodinator coordinate = new Coodinator(startX, startY, endX, endY);
